...
- **Mutate intentionally.** After proving a headline, break ONE load-bearing
  definition (consume doesn't decrement; remaining is always original; canonical
  args ignore authority context; error returns the recorded head; child debits
  full allocation not used share) and confirm the theorem FAILS. If it still
  proves, it wasn't the theorem you thought.
