...
//! Deny `assert!(value.ends_with(...))` loose matchers in punchlist production scope.
...
dylint_linting::declare_late_lint! {
    /// ### What it does
    ///
    /// Finds direct assertion conditions of the form `assert!(value.ends_with(expected))`.
    ///
    /// ### Why is this bad?
    ///
    /// Suffix assertions let unrelated output drift pass silently. Prefer exact
    ...
    pub ASSERT_ENDS_WITH,
    Deny,
    "loose `assert!(value.ends_with(...))` matcher"
}

impl<'tcx> LateLintPass<'tcx> for AssertEndsWith {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        if let ExprKind::MethodCall(segment, _receiver, args, _span) = expr.kind
            && is_ends_with_method(segment.ident.name.as_str(), args.len())
            && is_inside_assert_macro_invocation(cx, expr)
        {
            cx.emit_span_lint(
                ...
                rustc_errors::DiagDecorator(|diag| {
                    diag.primary_message("loose `assert!(value.ends_with(...))` matcher");
...
/// Returns true if the call shape matches `<value>.ends_with(<arg>)`.
fn is_ends_with_method(method_name: &str, arg_count: usize) -> bool {
    method_name == "ends_with" && arg_count == 1
}
...
mod tests {
    ...
    use super::{
        is_ends_with_method, is_inside_target_macro_invocation_at, is_production_path,
    ...
    #[test]
    fn ends_with_method_is_linted() {
        assert!(is_ends_with_method("ends_with", 1));
    }

    #[test]
    fn ends_with_method_without_argument_is_skipped() {
        assert!(!is_ends_with_method("ends_with", 0));
    }

    #[test]
    fn starts_with_method_is_skipped() {
        assert!(!is_ends_with_method("starts_with", 1));
    }

    #[test]
    fn direct_ends_with_call_is_inside_assert_macro_invocation() {
        let source = "assert!(output.ends_with(\"suffix\"));";
        // reason: the literal source string contains `output.ends_with` by construction.
        let byte_index = source.find("output.ends_with").expect("call exists");
    ...
    #[test]
    fn direct_ends_with_call_with_message_is_inside_assert_macro_invocation() {
        let source = "assert!(output.ends_with(\"suffix\"), \"bad suffix\");";
        // reason: the literal source string contains `output.ends_with` by construction.
        let byte_index = source.find("output.ends_with").expect("call exists");
    ...
    #[test]
    fn negated_ends_with_call_is_not_direct_assert_macro_condition() {
        let source = "assert!(!output.ends_with(\"suffix\"));";
        // reason: the literal source string contains `output.ends_with` by construction.
        let byte_index = source.find("output.ends_with").expect("call exists");
    ...
    #[test]
    fn nested_ends_with_call_is_not_direct_assert_macro_condition() {
        let source =
            "assert!(matches!(action, UsageError { message } if message.ends_with(\"--\")));";
        // reason: the literal source string contains `message.ends_with` by construction.
        let byte_index = source.find("message.ends_with").expect("call exists");
    ...
    #[test]
    fn ends_with_call_outside_assert_macro_invocation_is_skipped() {
        let source = "let configured = output.ends_with(\"suffix\");";
        // reason: the literal source string contains `output.ends_with` by construction.
        let byte_index = source.find("output.ends_with").expect("call exists");
