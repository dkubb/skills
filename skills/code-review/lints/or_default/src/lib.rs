...
//! Deny `.or_default()` fallback accessors in punchlist production scope.
...
dylint_linting::declare_late_lint! {
    /// ### What it does
    ///
    /// Finds `.or_default()` method calls.
    ///
    /// ### Why is this bad?
    ///
    /// Default substitution hides the absent computation from the caller.
    /// Prefer matching on the absent path explicitly so the fallback behavior
    /// is visible at the call site.
    ///
    /// ### Known problems
    ///
    /// This lint is syntactic. It does not resolve whether the receiver is a
    /// collection entry API or another type with a same-named method.
    pub OR_DEFAULT,
    Deny,
    "fallback `.or_default()` masks missing data"
}

impl<'tcx> LateLintPass<'tcx> for OrDefault {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        if let ExprKind::MethodCall(segment, _receiver, args, _span) = expr.kind
            && is_or_default_method(segment.ident.name.as_str(), args.len())
        {
            cx.emit_span_lint(
                OR_DEFAULT,
                expr.span,
                rustc_errors::DiagDecorator(|diag| {
                    diag.primary_message("fallback `.or_default()` masks missing data");
                    diag.help("match on the absent path explicitly");
                }),
            );
        }
    }
...
/// Returns true if the call shape matches `<value>.or_default()`.
fn is_or_default_method(method_name: &str, arg_count: usize) -> bool {
    method_name == "or_default" && arg_count == 0
}
...
mod tests {
    ...
    use super::{is_or_default_method, is_production_path, should_lint_target_source};
    ...
    #[test]
    fn or_default_method_is_linted() {
        assert!(is_or_default_method("or_default", 0));
    }

    #[test]
    fn or_default_method_with_argument_is_skipped() {
        assert!(!is_or_default_method("or_default", 1));
    }

    #[test]
    fn unwrap_or_default_method_is_skipped() {
        assert!(!is_or_default_method("unwrap_or_default", 0));
    }
}
