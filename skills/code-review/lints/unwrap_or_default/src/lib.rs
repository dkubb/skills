...
//! Deny `.unwrap_or_default()` fallback accessors in punchlist production scope.
...
dylint_linting::declare_late_lint! {
    /// ### What it does
    ///
    /// Finds `.unwrap_or_default()` method calls.
    ///
    /// ### Why is this bad?
    ///
    /// Default substitution hides the absent or failing computation from the
    /// caller. Prefer propagating the missing state through `Option`/`Result`,
    /// or model the absent case as a typed variant the caller must handle.
    ///
    /// ### Known problems
    ///
    ...
    Deny,
    "fallback `.unwrap_or_default()` masks missing data"
}

impl<'tcx> LateLintPass<'tcx> for UnwrapOrDefault {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        if let ExprKind::MethodCall(segment, _receiver, args, _span) = expr.kind
            && is_unwrap_or_default_method(segment.ident.name.as_str(), args.len())
        {
            cx.emit_span_lint(
                UNWRAP_OR_DEFAULT,
                expr.span,
                rustc_errors::DiagDecorator(|diag| {
                    diag.primary_message("fallback `.unwrap_or_default()` masks missing data");
...
/// Returns true if the call shape matches `<value>.unwrap_or_default()`.
fn is_unwrap_or_default_method(method_name: &str, arg_count: usize) -> bool {
    method_name == "unwrap_or_default" && arg_count == 0
}
...
mod tests {
    ...
    use super::{is_production_path, is_unwrap_or_default_method, should_lint_target_source};
    ...
    #[test]
    fn unwrap_or_default_method_is_linted() {
        assert!(is_unwrap_or_default_method("unwrap_or_default", 0));
    }

    #[test]
    fn unwrap_or_default_method_with_argument_is_skipped() {
        assert!(!is_unwrap_or_default_method("unwrap_or_default", 1));
    }

    #[test]
    fn unwrap_or_method_is_skipped() {
        assert!(!is_unwrap_or_default_method("unwrap_or", 0));
    }
}
